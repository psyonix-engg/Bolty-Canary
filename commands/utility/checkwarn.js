...
module.exports.run = async (client, message, args) => {
  ...
  if (!target) {
    ...
    var totalWarnsAuthor =
      warnDATA[message.guild.id][message.author.id].warnings;
    ...
    for (
      var i = 0;
      i < warnDATA[message.guild.id][message.author.id].dates.length;
      i++
    ) {
      var id = `#${i + 1}`;
      embed.addField(
        id,
        `\`${warnDATA[message.guild.id][message.author.id].dates[i]}\` | \`${
          warnDATA[message.guild.id][message.author.id].reasons[i]
        }\``
      );
    }
  ...
  var totalWarnsTarget = warnDATA[message.guild.id][target.user.id].warnings;
  ...
  for (var i = 0; i < warnDATA[message.guild.id][target.id].dates.length; i++) {
    ...
    embed.addField(
      id,
      `\`${warnDATA[message.guild.id][target.id].dates[i]}\` | \`${
        warnDATA[message.guild.id][target.id].reasons[i]
      }\``
    );
  }
